...
void find_best(node* n, void* data) {
  struct find_ctx* ctx = data;
  if (n->type == MY_D && n->size >= ctx->atleast) {
    if (n->size < ctx->best) ctx->best = n->size;
  }
}
...
int main(void) {
  ...
  ctx.atleast = 30000000 - (70000000 - root->size);
  ctx.best = root->size;
  walk(root, find_best, &ctx);
  printf("%d\n", ctx.best);
  return 0;
}
